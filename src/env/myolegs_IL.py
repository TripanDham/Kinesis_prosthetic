...
class MyoLegsIm(MyoLegsTask):
    ...
    def __init__(self, cfg):
        ...
        # Initialize GAIL Discriminator with History (t, t-1, t-2)
        self.history_len = cfg.env.get("history_len", 3)
        self.history_buffer = deque(maxlen=self.history_len)
        
        # Calculate observation size. We use the full observation for the discriminator.
        # Assuming get_obs_size() is available or can be calculated.
        # For now, let's use a dummy size or rely on the fact that we can set it via config.
        obs_size = self.get_obs_size() 
        self.gail_disc = GAILDiscrim(
            state_shape=(obs_size * self.history_len,),
            action_shape=(0,), # State-only
            hidden_units=cfg.env.get("gail_hidden_units", (256, 256))
        ).to(cfg.run.get("device", "cpu"))
        
        # Optimizer for discriminator (used by AgentGAIL)
        self.optim_disc = Adam(self.gail_disc.parameters(), lr=cfg.env.get("lr_disc", 3e-4))
    ...
    def setup_motionlib(self) -> None:
        ...
        self.motion_lib = ProstWalkCore(self.motion_lib_cfg)
    ...
    def reset_task(self, options: Optional[dict]=None) -> None:
        ...
        if self.test:
            if self.im_eval:
                ...
                if hasattr(self.cfg.run, "target_velocity") and self.cfg.run.target_velocity is not None:
                    self._sampled_motion_ids[:] = self.motion_lib.sample_motions_by_velocity(self.cfg.run.target_velocity)
                else:
    ...
    def compute_reward(self, action: Optional[np.ndarray] = None) -> float:
        ...
        # --- GAIL IMITATION REWARD WITH HISTORY (t, t-1, t-2) ---
        current_obs = self.get_obs() # Get full observation
        self.history_buffer.append(current_obs)
        
        # If buffer is not full yet, pad with the current observation
        while len(self.history_buffer) < self.history_len:
            self.history_buffer.append(current_obs)
            
        # Concatenate history [obs_{t-2}, obs_{t-1}, obs_t]
        hist_obs = np.concatenate(list(self.history_buffer))
        hist_tensor = torch.as_tensor(hist_obs, dtype=torch.float32, device=self.gail_disc.net[0].weight.device).unsqueeze(0)
        
        # Dummy action tensor (state-only GAIL)
        a_tensor = torch.zeros((1, 0), device=hist_tensor.device)
        
        with torch.no_grad():
            im_reward = self.gail_disc.calculate_reward(hist_tensor, a_tensor).item()
        
        reward += im_reward * self.reward_specs.get("w_gail", 0.1)
        # -------------------------------------------------------

        self.reward_info = reward_raw
        self.reward_info["upright_reward"] = upright_reward
        self.reward_info["energy_reward"] = energy_reward
        self.reward_info["imitation_reward_gail"] = im_reward
